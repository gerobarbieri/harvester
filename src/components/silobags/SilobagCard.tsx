    ...
    return (
        <Card className={`p-4 transition-all duration-300 group ${isClosed ? 'bg-gray-100 opacity-80' : 'bg-surface hover:shadow-md hover:border-gray-300'}`}>
            <div className="flex items-center gap-4">
                ...
                {/* --- 2. Contenido Principal (Linkeable) --- */}
                <Link to={`/silo-bags/${silo.id}`} className="flex-1 min-w-0">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="font-bold text-text-primary truncate group-hover:text-primary">{silo.name}</p>
                            <p className="text-sm text-text-secondary">
                                {silo.crop.name} <span className="text-gray-300 mx-1">•</span> {silo.field.name}
                            </p>
                        </div>
                        <div className="text-right flex-shrink-0 ml-2">
                            <p className="font-bold text-lg text-primary">{hasLoss ? '0' : formatNumber(silo.current_kg)}</p>
                    ...
                    {hasLoss ? (
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-1.5 mt-2 overflow-hidden" title={`${fillPercentage.toFixed(0)}%`}>
                            <div style={{ width: `${isClosed ? 0 : fillPercentage}%` }} className="h-full bg-primary-darker rounded-full transition-all duration-500"></div>
                        </div>
                    )}
                </Link>

                {/* --- 3. Acciones (Fuera del Link) --- */}
                {!isClosed && (
                    <div className="flex items-center border-l border-gray-200 pl-3">
                        <Button variant="ghost" className="!p-2" onClick={onExtract} title="Extraer Kilos">
                            <MinusCircle size={20} className="text-text-secondary hover:text-primary" />
                        </Button>
                        <Button variant="ghost" className="!p-2" onClick={onClose} title="Cerrar Silo">
