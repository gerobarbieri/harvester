...
const SiloBagCard = ({ silo, onExtract, onClose }) => {
    const fillPercentage = silo.initialKg > 0 ? (silo.currentKg / silo.initialKg) * 100 : 0;
    const isClosed = silo.currentKg <= 0;
        ...
        <Card className={`transition-all duration-300 flex flex-col justify-between ${isClosed ? 'bg-gray-50 opacity-70' : 'bg-surface'}`}>
            <div>
                <div className="flex justify-between items-start">
                    <div>
                        <h3 className="font-bold text-lg text-text-primary">{silo.id}</h3>
                        <p className="text-sm text-text-secondary">{silo.crop} - {silo.creationDate}</p>
                    </div>
                    {isClosed && <span className="px-3 py-1 text-xs font-bold text-red-800 bg-red-100 rounded-full">CERRADO</span>}
                </div>
                <div className="my-4 text-center">
                    <p className="text-4xl font-bold text-primary-dark">{silo.currentKg.toLocaleString()}</p>
                ...
                <div>
                    <div className="h-3 w-full bg-background rounded-full overflow-hidden">
                        <div style={{ width: `${fillPercentage}%` }} className="h-full bg-primary transition-all duration-500"></div>
                    </div>
                    <p className="text-xs text-right mt-1 text-text-secondary">Inicial: {silo.initialKg.toLocaleString()} kg</p>
                </div>
            </div>
            {!isClosed && (
